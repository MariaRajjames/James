...
class Tableitem extends React.Component{
    ...
    render(){
        ...
        if(this.state.movies.length == 0){
            Element = <div>its End</div>
            Hide = <div></div>
        }else{
            Element = <div>No of Movie is: {this.state.movies.length}</div>
            Hide = <table class="table" >
                    <thead>
                            <tr>
        ...
        return(
            <div>
                <p>{Element}</p>
                <h5>{Hide}</h5>        
                {this.state.movies.map((movie)=><Tablelist key={movie.id} id={movie.id} title={movie.title} year={movie.year} runtime={movie.runtime} actors={movie.actors} onDelete={this.handleClick} />)}
            </div>
        );
    }
}
